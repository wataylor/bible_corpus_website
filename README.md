...
**http://perennitypublishing.com/corpus/** and
**http://perennitypublishing.com/1/**

## Usage and Overview ##

**index.html** has a set of radio buttons which specifies which translation language comes first and a set of check boxes which select additional translations.  The reference box accepts a Bible reference like **Ge. 1:1-10** which tells what passage to display for the selected language(s).
...
The program also writes an error file for each Bible file that specifies verses that do not exist.  For example, the Piate Bible supplies a text string for verse **3JO.1.15** but 3rd John doesn't have verse 15, it ends with verse 14.

## Build and Deploy ##

This is a Maven **war** project (see [pom.xml](pom.xml)).  The packaged WAR is intended for a servlet container such as Tomcat.

Typical flow:

1. Build the WAR with Maven.
2. Deploy the WAR to your servlet container.
3. Ensure the database configuration file (see next section) is available on the application classpath.

The primary UI is served from [WebContent/index.jsp](WebContent/index.jsp).  A placeholder [src/main/webapp/index.html](src/main/webapp/index.html) exists for the Maven webapp folder but is not used by the JSP-based UI.

## Database Configuration ##

Database connection information is loaded by `DBProp.PropertyConnection("Perennity")` in [src/main/java/asst/dbase/DBProp.java](src/main/java/asst/dbase/DBProp.java).  This requires a **Perennity.properties** file on the classpath at runtime.  The file is not in this repository and must be provided when deploying.

Expected keys (names are taken from `DBProp`):

- **Perennity.dbaseDriver**
- **Perennity.dbaseURL** (include JDBC URL and any connection parameters)
- **Perennity.dbaseUser**
- **Perennity.dbasePassword**
- **Perennity.dbaseOwner**
- **Perennity.dbaseSequence**
- **Perennity.dbaseRattler**
- **Perennity.dbaseProduct** (MySQL, MSSQLServer, or Oracle)
- **Perennity.debug** (optional, set to `true` for extra logging)

For MySQL, the URL typically looks like:

`jdbc:mysql://<host>/<db>?user=<uid>&useUnicode=yes&characterEncoding=UTF-8`

## Web Endpoints ##

- **/index.jsp**: Main UI with language radios and checkboxes.  Generated from `MapLanguagesToTables` and client-side scripts.
- **/getRef.jsp**: Handles passage lookup and renders HTML results (see [WebContent/getRef.jsp](WebContent/getRef.jsp)).
- **/ProduceHtml**: Servlet that generates (or returns) a full HTML file for a single translation, using templates in [src/main/java/asst/corpus/utils/start.html](src/main/java/asst/corpus/utils/start.html) and [src/main/java/asst/corpus/utils/end.html](src/main/java/asst/corpus/utils/end.html) and writing to the configured output path (see [src/main/java/asst/corpus/ProduceHtml.java](src/main/java/asst/corpus/ProduceHtml.java)).

## Adding a Translation ##

When a new translation is added:

1. Create/alter the target table and add the column (see [aText.sql](aText.sql) and [src/test/java/asst/loaders/BuildColumsAndRadiosMain.java](src/test/java/asst/loaders/BuildColumsAndRadiosMain.java)).
2. Load verse data into the new column (see [src/test/java/asst/loaders/LoadCorpusBibleFilesMain.java](src/test/java/asst/loaders/LoadCorpusBibleFilesMain.java)).
3. Update `LANG_2_TABLE` in [src/main/java/asst/MapLanguagesToTables.java](src/main/java/asst/MapLanguagesToTables.java).
4. Update the client-side list of translations in [WebContent/javascript/bookNames.js](WebContent/javascript/bookNames.js) and regenerate any radio/checkbox fragments if you use the generator in [src/test/java/asst/loaders/BuildColumsAndRadiosMain.java](src/test/java/asst/loaders/BuildColumsAndRadiosMain.java).

## Troubleshooting ##

- **Missing properties file**: `DBProp.PropertyConnection` throws an error when **Perennity.properties** is not found or is missing required keys.  Ensure it is on the classpath.
- **Character encoding issues**: Make sure your MySQL tables and columns are created with UTF-8 (see the `aText.sql` example and the `SET CHARACTER SET 'utf8'` call in [src/main/java/asst/corpus/ProduceHtml.java](src/main/java/asst/corpus/ProduceHtml.java)).
- **No verses returned**: Check that the translation is mapped in `LANG_2_TABLE` and that the target table contains populated rows for the requested book/chapter/verse.